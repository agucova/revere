...
# ╔═╡ 17bb2fe1-3714-405b-bc30-bd66b95efdda
begin
	using JSServe
	Page(exportable=true)
end

# ╔═╡ 3706157c-f0bc-11eb-2980-d9b7baab39fc
using DataFrames, CSV, LinearAlgebra, LightGraphs, GraphPlot, Colors

# ╔═╡ 5008fdb3-19a9-4ad9-8456-0dc9e4465d92
using SimpleWeightedGraphs

# ╔═╡ c135a77c-d1f4-4a57-a97a-57d5b1f02f0d
using WGLMakie, GraphMakie, GraphMakie.NetworkLayout

# ╔═╡ 8ebc3759-2ba4-4614-a87e-5a18708226fc
WGLMakie.activate!()

# ╔═╡ 1eb58f7f-565f-49ce-8469-7a8ec05abf80
df = DataFrame(CSV.File("PaulRevereAppD.csv"))

# ╔═╡ 38e391f9-ae81-43aa-b5f9-2d3397dc7589
A = Matrix{Int64}(df[:, 2:end])

# ╔═╡ ebd25456-f2c4-467d-8316-7ece92633cc1
P = A * transpose(A)

# ╔═╡ 66ba55b7-a05b-45f6-9cfd-4f2d05875d23
g = transpose(A) * A

# ╔═╡ 3c8e708a-3acd-40ba-bbc7-c6e62f1ab6db
P_graph, g_graph = SimpleWeightedGraph(P), SimpleWeightedGraph(g)

# ╔═╡ 98e0fac8-6402-46b6-8265-0cb2446d8c36
colors(n) = range(HSL(colorant"slateblue2"), stop=HSL(colorant"deepskyblue"), length=n)

# ╔═╡ 3dc8d23a-253c-4b16-9bef-307c8195f308
md"""## Let's plot it"""

# ╔═╡ c6fc7394-fd3a-441c-954a-29790d600cfb
# gplot(P_graph, nodefillc=colors(nv(P_graph)))

# ╔═╡ c69011be-024c-4727-a271-57091b90574b
# gplot(g_graph, nodelabel=names(df)[2:end], nodefillc=colors(nv(g_graph)), nodelabelc=colorant"black", layout=circular_layout)

# ╔═╡ 5edc37f6-4b27-4154-ae16-2265c8f29adc
set_theme!(resolution=(650, 800))

# ╔═╡ 6507f4ef-d50e-422f-b68e-3725d5deab24
graphplot(P_graph, nlabels=df[:, 1], nlabels_textsize=4, node_color=colorant"black", edge_color=colorant"rgba(0,60,120,0.15)", layout=Spring(dim=3, seed=5))

# ╔═╡ d62f0c3c-b62e-40d7-9595-e8b9de38c887
graphplot(P_graph, nlabels=df[:, 1], nlabels_textsize=4, node_color=colorant"black", edge_color=colorant"rgba(0,60,120,0.15)")

# ╔═╡ 1a0ceef2-84cf-4080-9471-a0a81eca4f00
df[argmax(betweenness_centrality(P_graph)), 1]
